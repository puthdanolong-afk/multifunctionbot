...
/stats - View your usage statistics`;
    ...
    bot.action(/^(ocr_photo|pdf_photo|ocr_pdf|img_pdf)_(.+)$/, async (ctx) => {
      ...
      try {
        ...
        if (action === 'ocr_photo' || action === 'ocr_pdf') {
          ...
          try {
            ...
            const aiResponse = await generateContentWithRetry(ai, {
              ...
              contents: [
                ...
                'Extract all text from this document. Preserve the original formatting, layout, spacing, and line breaks as accurately as possible. If it\'s a table, format it as a markdown table. If there is no text, describe the image.'
              ]
            });
            await ctx.telegram.deleteMessage(ctx.chat.id, statusMsg.message_id).catch(() => {});
            ...
            if (aiErr.message?.includes('503') || aiErr.message?.includes('UNAVAILABLE')) {
          ...
          await ctx.telegram.editMessageText(ctx.chat.id, statusMsg.message_id, undefined, '🔄 *Step 2/2:* Converting PDF pages to images... (Max 10 pages)', { parse_mode: 'Markdown' }).catch(() => {});
          ...
          for (let i = 1; i <= numPages; i++) {
            ...
            await ctx.replyWithPhoto({ source: imgBuffer }, { caption: `Page ${i}` });
          }
          await ctx.telegram.deleteMessage(ctx.chat.id, statusMsg.message_id).catch(() => {});
    ...
    bot.command('tts', async (ctx) => {
      ...
      try {
        ...
        if (!text) {
          return ctx.reply('Please provide text. Example: /tts Hello world');
        }
        if (ctx.from) {
          userManager.incrementStat(ctx.from.id, 'audioGenerated');
        }
        const statusMsg = await ctx.reply('⏳ Generating audio... (This may take a few seconds)');

        const aiResponse = await generateContentWithRetry(ai, {
          model: "gemini-2.5-flash",
          contents: [{ parts: [{ text: `Please generate audio for the following text:\n\n${text}` }] }],
          config: {
    ...
    const processVideoDownload = async (ctx: any, url: string, editMessageId?: number) => {
      ...
      try {
        ...
        if (url.includes('tiktok.com')) {
          ...
          const tikResponse = await axios.post('https://www.tikwm.com/api/', `url=${url}`, {
            headers: {
          ...
          // Skip yt-dlp for YouTube and go straight to Cobalt API to avoid bot detection hangs
        } else {
          // Use yt-dlp to download directly for best quality (Facebook, Instagram, etc.)
          try {
            ...
            const isBotDetection = ytErr.message && (
              ytErr.message.includes('Sign in to confirm you’re not a bot') || 
              ytErr.message.includes('Sign in to confirm you')
            );
            
            if (isBotDetection) {
               // YouTube bot detection triggered in yt-dlp, falling back to Cobalt API
        ...
        if (videoUrl || downloadedFilePath) {
          ...
          if (sizeMB > 49) {
            ...
            if (downloadedFilePath) {
              ...
              await ctx.replyWithVideo({ source: downloadedFilePath });
              fs.unlinkSync(downloadedFilePath);
            } else {
