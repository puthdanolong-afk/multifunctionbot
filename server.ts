    ...
    bot.on(message('photo'), async (ctx) => {
      ...
      if (msg.media_group_id) {
        ...
        group.timer = setTimeout(async () => {
          ...
          // Clear map automatically to prevent memory leak
          setTimeout(() => {
...
- Output ONLY the extracted text, without any conversational filler.`;
          ...
          for (let i = 1; i <= numPages; i++) {
            ...
            // Calculate a scale to make the largest dimension around 1600 pixels to balance quality and memory
            let scale = 1600 / maxDim;
            scale = Math.max(1.0, Math.min(scale, 3.0)); // keep scale within reasonable bounds
            ...
            // Generate JPEG to save memory (PNG uses significantly more memory and crashes Render free tier)
            const imgBuffer = canvas.toBuffer('image/jpeg', { quality: 0.90 });
            ...
            mediaGroup.push({
              type: 'document',
              media: { source: imgBuffer, filename: `Page_${i}.jpg` },
              caption: `Page ${i}`
            });

            // Send in smaller chunks of 5 to keep memory low 
            if (mediaGroup.length === 5 || i === numPages) {
              await ctx.replyWithMediaGroup(mediaGroup);
              mediaGroup = [];
              // Yield to event loop and give GC a chance to run
              await new Promise(resolve => setTimeout(resolve, 50));
            }
          }
          pdfDocument.destroy(); // Free memory
          await ctx.telegram.deleteMessage(ctx.chat.id, statusMsg.message_id).catch(() => {});
      ...
      try {
        ...
        if (cleanUrl.includes('tiktok.com')) {
          ...
          try {
            ...
            if (!isBotDetection) {
              console.error('yt-dlp Error details:', ytErr.message || ytErr);
              if (ytErr.stderr) console.error('stderr:', ytErr.stderr);
            }
            
            if (isBotDetection && isYoutube) {
               console.log('yt-dlp bot detection triggered on YouTube, informing user about cookies gracefully.');
        ...
        if (videoUrl || downloadedFilePath) {
          ...
          if (sizeMB > 49) {
            ...
            // Cleanup file and map automatically after 1 hour avoiding memory leaks
            setTimeout(() => {
              const video = pendingLargeVideos.get(shortId);
        ...
        // Clear map automatically to prevent memory leak
        setTimeout(() => {
          pendingVideoRetries.delete(shortId);
        }, 60 * 60 * 1000); // 1 hour timeout
